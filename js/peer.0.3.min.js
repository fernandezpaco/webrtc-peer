/*! peerjs_fork_firefox40 build:0.3.15, development. Copyright(c) 2013 Michelle Bu <michelle@michellebu.com> */
/*! peerjs_fork_safari TP */
...
Negotiator.startConnection = function(connection, options) {
  ...
  if (connection.type === 'media' && options._stream) {
    ...
    if(pc.addStream){
      ...
      options._stream.getTracks().forEach(track => pc.addTrack(track, options._stream));
    }
  }
...
Negotiator._makeOffer = function(connection) {
  ...
  var onSuccess = function(offer) {
   ...
   var localDesc = function() {
    ...
    var localDescErr = function(err) {
      connection.provider.emitError('webrtc', err);
      util.log('Failed to setLocalDescription, ', err);
    };

    pc.setLocalDescription(offer).then(localDesc).catch(localDescErr);
  };
  ...
  if( pc.createOffer(connection.options.constraints) instanceof Promise){
    pc.createOffer(connection.options.constraints).then(onSuccess).catch(onError);  
  }else{
    pc.createOffer(onSuccess,onError,connection.options.constraints); 
  }
...
Negotiator._makeAnswer = function(connection) {
  ...
  var localAnswer = function(answer) {
    ...
    var localError = function(err) {
      connection.provider.emitError('webrtc', err);
      util.log('Failed to setLocalDescription, ', err);
    };


  pc.setLocalDescription(answer).then(localSuccess).catch(localError);
  };

  var localAnswerError = function(err) {
    connection.provider.emitError('webrtc', err);
    util.log('Failed to create answer, ', err);
  };

  pc.createAnswer().then(localAnswer).catch(localAnswerError);
}
